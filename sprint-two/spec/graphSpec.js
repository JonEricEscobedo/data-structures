describe('graph', function() {
  ...
  it('*NEW* should remove edges among multiple nodes when a node is removed', function() {
    graph.addNode(1);    
    graph.addNode(2);  
    graph.addNode(3);
    graph.addNode(4);
    ...
    expect(graph.hasEdge(1, 4)).to.equal(true);
  ...
  it('*NEW* should remove ALL connected edges in one shared node (node # 5 is connected to all other nodes)', function() {
    graph.addNode(1);    
    graph.addNode(2);  
    graph.addNode(3);
    graph.addNode(4);
